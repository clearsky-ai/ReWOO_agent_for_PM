...
def variants(file_path):
    """ Executes process discovery using the variants approach """
    if "csv" in file_path:
        ...
        file_type = "xes"
    variants_abstraction = (pm4py.llm.abstract_variants(event_log))

    # Visualizing process models using the preferred format
    model_visualizer(event_log)

    # Visualizing analytics according to set preferences
    ...
    prompt = filled_template + variants_abstraction
